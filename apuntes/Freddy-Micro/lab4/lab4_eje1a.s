...
reset:
    ...
    LDR R1,=TIMER0_BASE
    MOV R0,#0x00
    STR R0,[R1,#CTCR]  //Escribo un 0 en el registro de control (Uso clock interno)

    // Prescaler de 9.500.000 para una frecuencia de 10 Hz, el procesador tiene 95Mhz
    LDR R0,=9500000
    STR R0,[R1,#PR]  //Cuento hasta 10 por cada segundo

    // El valor del semperiodo para 1 Hz
    LDR R0,=5        //Si cuento hasta 10 por cada segundo, contando hasta 5 tengo medio segundo
    STR R0,[R1,#MR3] //Pongo este 5 en el registro de match3

    // El registro de match 3 provoca reset del contador
    MOV R0,#(MR3R | MR3I)  //Con MR3R pido reset, y con MR3I pido interrupcion
    STR R0,[R1,#MCR]  //Lo cargo en registro de control de match

    // Limpieza del contador
    MOV R0,#CRST
    STR R0,[R1,#TCR] //Escribo en el TCR un reset

    // Inicio del contador
    MOV R0,#CEN
    STR R0,[R1,#TCR]
    //Hasta acá el contador esta funcionando, pero falta habilitar la interrupcion

    // Limpieza del pedido pendiente en el NVIC, lipio pedidos viejos
    LDR R1,=NVIC_ICPR0  //Registro 0 maneja de la 0 a la 31
    MOV R0,(1 << 12)  //La entrada 28 del vector, que es la interrupcion del timer 0, corresponde a la IRQ 12 en el NVIC
    STR R0,[R1]
    ...
    LDR R3,=TIMER0_BASE
    LDR R0,[R3,#IR]  //Leo el TIR y lo guardo en R0
    STR R0,[R3,#IR]  //Lo vuelvo a escribir para borrarlo
