...
hora:
    .byte 0x04,0x02         // Vector correspondiente las HH:MM:SS
...
disminuir:
    LDR R8,=hora  //Apunto al segundo S0
    LDRB R6,[R8] //Cargo en R10 el valor del segundo S0
    CMP R6,#0
    BNE decrementoS0
    LDR R8,=hora+1//Apunto al segundo S1
...
break: LDR R7,=hora  //Apunto R7 a la direccion del vector del reloj
       ...
       CMP R6,#1      //Bandera para ver que display encender
       BHI cerar      //Si estoy en el tercer display (que no existe) reinicio la bandera
       B salir_mostrar
cerar:
    MOV R6,#0
salir_mostrar:
